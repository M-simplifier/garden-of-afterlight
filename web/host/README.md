...
Open that site, click Start (audio requires a user gesture), and inspect
`window.afterlightWeb` for frame/FFI counts, CPU frame duration, layout, and
current memory bytes. `?autostart` is for non-audio smoke runs. Metrics count
...
the game's desired mode, and only a trusted Start/canvas click requests Pointer
Lock. A denial is logged and a later canvas click can retry. Escape never
automatically reacquires the mouse. The browser's pointer-lock event still
updates raylib's actual cursor state. Other runtime failures remain fatal.
F11 requests browser fullscreen inside the original key event. Game shortcut
defaults (including Tab, F3, F5, F6 and F12) are suppressed only while the canvas
has focus; the events continue to raylib and Haskell without state injection.

## Assets and checkpoint storage

Provide `public/assets-manifest.json` as `[{"path":"assets/fonts/glyphs.txt"},
...
Defaults are 1280 × 720. Save paths and arbitrary environment variables cannot
