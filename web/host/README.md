...
CSS fills the viewport. `drawingSize` defaults to CSS size × device pixel ratio,
preserving display detail rather than silently upscaling a smaller buffer.
The ready/paused controls offer high (100%), standard (75%) and light (50%)
scene render scales. `setQuality(app, percent)` selects a Haskell `RenderQuality`
at warmup and on changes. Only the existing 3D render texture is scaled; its
post-processed image fills the unchanged canvas. HUD, menus and absolute input
coordinates stay at display density. The choice persists under `afterlight:render-scale:v1`, separately
from checkpoints; unavailable preference storage is nonfatal. Texture and
renderbuffer dimension limits are queried once after context creation and
constrain the size while preserving aspect ratio. Resize or a changed DPR updates
the actual raylib buffer; `preview` redraws a ready scene without advancing the world.
...
change either of them. Pointer Lock deltas are already relative movement in
the pinned Emscripten/raylib path: do not multiply them by DPR or render scale.
Unlocked absolute menu coordinates still use the canvas/CSS conversion.
...
existence. Fetches run four at a time independently of raylib initialization;
the completed bytes are installed once the filesystem is available. A missing manifest (404) means an
...
Default size follows the viewport and display density; the default state
...
For a bounded scene-resolution comparison, open `?diagnostics=1&render-check=1` and
leave the scene ready. The optional `render-check.js` sampler redraws the same
scene, discards ten settling frames, then reports median/p95 CPU call duration
and rAF interval over 90 frames in that DOM snapshot. A resize or quality change
starts a new sample. It does not advance simulation or write saves. Compare the
same viewport, DPR and scene; these are neither GPU timings nor a gameplay FPS
benchmark. Normal visits do not run the sampler.
